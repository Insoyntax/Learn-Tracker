...
export async function GET() {
  ...
  try {
    ...
    const rows = await sql`
      ...
      FROM projects WHERE user_id = ${userId} ORDER BY id LIMIT 100`;
    return ok(rows);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { title, description, image_url, demo_url, category_id } = body;
    ...
    const rows = await sql`
      ...
      VALUES (${title.trim()}, ${description ?? ""}, ${image_url ?? null}, ${demo_url ?? null}, ${category_id ?? null}, ${userId})
